package botsandbytes.java.backend.template.builder;

import com.google.common.collect.Sets;

import botsandbytes.java.backend.template.filter.ColumnFilter;
import botsandbytes.java.backend.template.filter.NumberColumnFilter;
...
public class SqlQueryBuilder {
	...
	public String createSql(GetRowsRequest request, String tableName, Map<String, List<String>> pivotValues) {
	...
	private String limitSql() {
		// return " OFFSET " + startRow + " ROWS FETCH NEXT " + (endRow - startRow + 1)
		// + " ROWS ONLY";
	...
	private Stream<String> extractPivotStatements() {
		...
		return Sets.cartesianProduct(pivotPairs).stream().flatMap(pairs -> {
			...
			String decodeStr = pairs.stream()
					.map(pair -> "case when " + pair.getLeft() + "='" + pair.getRight() + "' then ")
					.collect(joining(", "));

			String closingBrackets = IntStream.range(0, pairs.size()).mapToObj(i -> " end) as ").collect(joining(""));
	...
	private Map<String, String> operatorMap = new HashMap<String, String>() {
		private static final long serialVersionUID = -2632056181870249606L;
